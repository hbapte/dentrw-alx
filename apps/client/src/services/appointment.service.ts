...
const AppointmentService = {
  ...
  getAppointmentsByDoctorId: async (doctorId: string, params?: AppointmentPaginationParams): Promise<PaginatedAppointmentsResponse> => {
    const queryParams = new URLSearchParams()
    if (params?.page) queryParams.append('page', params.page.toString())
    if (params?.limit) queryParams.append('limit', params.limit.toString())
    const url = `/appointments/by-doctor/${doctorId}${queryParams.toString() ? `?${queryParams.toString()}` : ''}`
    const response = await api.get(url)
    return {

...
const profileService = {
  ...
  updateAdminProfile: async (data: AdminProfileData) => {
    try {
      ...
      const errorData: ProfileError = {
        status: error.response?.status,
        message: error.response?.data?.error?.message || "Failed to update admin profile",
        details: error.response?.data?.error?.details,
      }
      throw errorData
    }
  ...
  updateReceptionistProfile: async (data: ReceptionistProfileData) => {
    try {
      const response = await api.put("/profile/receptionist", data)
      return response.data
    } catch (error: any) {
      const errorData: ProfileError = {
        status: error.response?.status,
        message: error.response?.data?.error?.message || "Failed to update receptionist profile",
        details: error.response?.data?.error?.details,
      }
      throw errorData
    }
